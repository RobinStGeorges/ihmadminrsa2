...
export class EquipementComponent implements OnInit {
  ...
  trackId = (_index: number, item: IEquipement): string => this.equipementService.getEquipementIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.equipements = this.refineData(dataFromBody);
  }

  protected refineData(data: IEquipement[]): IEquipement[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IEquipement[] | null): IEquipement[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.equipementService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
